...
def run_trial_maintenance():
    ...
    try:
        ...
        for k in keys:
            ...
            cur2 = conn.cursor()
            cur2.execute(
                "INSERT INTO trial_reminder_state (api_key_id, sent_3d, sent_2d, sent_1d) VALUES (%s, FALSE, FALSE, FALSE) ON CONFLICT (api_key_id) DO NOTHING",
                (api_key_id,),
            )
            conn.commit()
            cur2.close()
            ...
            if remaining_days in remind_map:
                ...
                cur2 = conn.cursor()
                cur2.execute(f"UPDATE trial_reminder_state SET {flag}=TRUE WHERE api_key_id=%s", (api_key_id,))
                conn.commit()
                cur2.close()
